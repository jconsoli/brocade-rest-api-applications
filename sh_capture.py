#!/usr/bin/python
# -*- coding: utf-8 -*-
# Copyright 2021, 2022, 2023 Broadcom Communications, Inc.  All rights reserved.
#
...
__copyright__ = 'Copyright 2021, 2022, 2023 Broadcom Communications, Inc.'
...
_DEBUG_i = 'sac/gregory_bricker_221206_1556_SAC'  # Name of SAN Health report.
...
def _process_zone(sheet_d):
    ...
    if len(ml) > 0:  # A zone configuration may not be active
        brcddb_util.add_to_obj(_working_fab_obj, brcdapi_util.bz_eff_cfg,
                               ml[0]['val'].split('"')[1])
        buf = ml[0].get('val')
        if buf is not None:
            tl = buf.split('"')
